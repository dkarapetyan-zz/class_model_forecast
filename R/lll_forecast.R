...
LLLForecast <- function(position_data, model_coefficients, macro_forecasts) {
	...
	for (i in 1:(.row - 4)) {
		if (i == 1) {
			.lll_forecast_df$Total.Reserves[i] <-
					position_data$Total.Reserves
		} else {
			...
			.lll_forecast_df$Total.Reserves[i] <-
					if (is.na(
							.lll_forecast_df$Total.Reserves[i - 1] -
									.lll_forecast_df$Total.Net.Charge.offs[i] <
									.lll_forecast_df$X4.Qrt.Net.Charge.offs[i]))
					{
						.lll_forecast_df$Total.Reserves[i - 1] 
					}					else if ((.lll_forecast_df$Total.Reserves[i - 1] -
								.lll_forecast_df$Total.Net.Charge.offs[i]) <
							.lll_forecast_df$X4.Qrt.Net.Charge.offs[i]) {
						.lll_forecast_df$X4.Qrt.Net.Charge.offs[i]
					} else if ((.lll_forecast_df$Total.Reserves[i - 1] -
