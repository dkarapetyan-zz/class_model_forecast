...
#load the ratios
ratio.input.data1=read.csv("Ratio.csv")
dim(ratio.input.data1)
names(ratio.input.data)
names(ratio.input.data1)


#load the coeffcients
coeff.data=read.csv("CLASS_Market_Data_Input_Intercept.csv")

#load the current PPNR and NCO data
#Output.current=read.csv("response.csv")
#dim(Output.current)
...
colnames(forecast.PPNR.base)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")

		for (i in c(1,2,4:7))  #loop for PPNR	
	{
		#step 0 set up the output matrix for each line item
          y.hat.mat=matrix(NA, ncol=(T.forecast+2), nrow=201)       #loop of the forecasting period
	  
       #step 1- read the response @stepoff point -- 2014Q3 in this experiment
        #@t=1
...
colnames(forecast.NCO.base)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")


		for (i in c(3,8:22))  #loop for NCO	
	{
		#step 0 set up the output matrix for each line item
...
colnames(forecast.PPNR.adverse)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")
...
colnames(forecast.NCO.adverse)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")
...
colnames(forecast.PPNR.severely.adverse)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")
...
colnames(forecast.NCO.severely.adverse)=c(as.character(macro.data[,"Date"]),"SNL.Institution.Key","Variable")
