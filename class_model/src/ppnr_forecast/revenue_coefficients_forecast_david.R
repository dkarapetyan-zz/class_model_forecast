...
RevenueCoeffForecast<- function(position.data, model.coefficients, macro.forecasts ) {
	...
	.required_colnames_macro <-
			c("Real.GDP.growth", "Nominal.GDP.growth", "Real.disposable.income.growth", 
					"Nominal.disposable.income.growth", "Unemployment.rate", 
					"CPI.inflation.rate", "X3.Month.Treasury.Yield", "X5.year.Treasury.yield", 
					"X10.year.Treasury.yield", "BBB.corporate.yield", "Mortgage.rate", 
					...
					"Home.price.growth.if.growth.is.negative", "Commercial.Property.Price.Growth.Negative", 
...
#create blank capital forecast time series
	...
	for(i in 1:.nrows) { 
		if (i==1) { #initial data for ppnr coefficients. Comes from arithmetic on position data
			...
			.revenue.coeffs.forecast.ts[i, "Return.on.Trading.Assets"] <-  (
			...
			.revenue.coeffs.forecast.ts[i, "Net.Interest.Margin" ] <-
					(model.coefficients["Intercept", "Net.Interest.Margin"]
						...
						*.revenue.coeffs.forecast.ts[(i-1), "Net.Interest.Margin"]
			...
			.revenue.coeffs.forecast.ts[i, "Fixed.Asset.Noninterest.Expense.Ratio"] <- (
						...
						*.revenue.coeffs.forecast.ts[(i-1), "Fixed.Asset.Noninterest.Expense.Ratio"]
			...
			.revenue.coeffs.forecast.ts[i, "Other.Noninterest.Expense.Ratio"] <- (
						...
						*.revenue.coeffs.forecast.ts[(i-1), "Other.Noninterest.Expense.Ratio"]
			...
			.revenue.coeffs.forecast.ts[i, "Return.on.Trading.Assets"] <- (
						...
						*.revenue.coeffs.forecast.ts[(i-1), "Return.on.Trading.Assets"]
	...
	return (.revenue.coeffs.forecast.ts)
}
