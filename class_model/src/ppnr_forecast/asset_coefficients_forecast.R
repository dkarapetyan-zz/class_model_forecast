...
RevenueCoeffForecast <- function(position_data, model_coefficients, macro_forecasts) {
  ...
  .rownames <- c("Residential.RE.Loans.Ratio",
  ...
  .position_data_processed <- data.frame(matrix(NA, nrow = length(.rownames), ncol = length(.colnames),
  ...
  for (str in .colnames) {
    if (str == "B.S.Ratios") {
      ...
      .position_data_processed[!(row.names(.position_data_processed)
                %in% c("Asset.Share")), "B.S.Ratios"] <-
          as.numeric(position_data[.pd_subset1]/sum(position_data[.pd_subset2]) * 100)
      
      # take care of Asset.Share
      .position_data_processed["Asset.Share", "B.S.Ratios"] <- position_data$Asset.Share
    } else {
      # looping portion
      .position_data_processed[str] <- (
            model_coefficients[.rownames, str] * .position_data_processed$B.S.Ratios
            )
    }
  }
  ...
  .revenue_coeffs_forecast_df <- data.frame(ts(matrix(NA, ncol = length(cols)),
  ...
  #Extracting time series start end and frequency from macro_forecasts. Now convert to
# data frame for easier manipulation in ensuing computations 
  ...
  for (i in 1:.nrows) {
    if (i == 1) {
      ...
      .revenue_coeffs_forecast_df$Net.Interest.Margin[i] <-
          position_data$Net.Interest.Income...000./sum(position_data[.summing_vec]) * 400
      
      .revenue_coeffs_forecast_df$Noninterest.Nontrading.Income.Ratio[i] <-
          ((position_data$Total.Noninterest.Income...000.
              - position_data$NII..Trading.Revenue...000.)/position_data$Total.Assets...000. * 400)
      
      .revenue_coeffs_forecast_df$Compensation.Noninterest.Expense.Ratio[i] <-
          (position_data$NIE..Salary...Benefits...000./position_data$Total.Assets...000. * 400)
      
      .revenue_coeffs_forecast_df$Fixed.Asset.Noninterest.Expense.Ratio[i] <-
          (position_data$NIE..Premises...Fixed.Assets...000./position_data$Total.Assets...000. * 400)
      ...
      .revenue_coeffs_forecast_df$Other.Noninterest.Expense.Ratio[i] <- (
            ((position_data$Total.Noninterest.Expense...000.
                - sum(position_data[c("NIE..Salary...Benefits...000.", 
                            "NIE..Premises...Fixed.Assets...000.")]))/position_data$Total.Assets...000.) * 400)
      
      .revenue_coeffs_forecast_df$Return.on.Trading.Assets[i] <- (
            position_data$NII..Trading.Revenue...000./position_data$Total.Trading.Assets...000.
            * 400)
    } else {
      # arithmetic
      
      .revenue_coeffs_forecast_df$Net.Interest.Margin[i] <- (
            model_coefficients["Intercept", "Net.Interest.Margin"] + model_coefficients["Lagged.dependent.variable", 
                "Net.Interest.Margin"] * .revenue_coeffs_forecast_df$Net.Interest.Margin[i-1] + 
            ...
            * macro_forecasts$X3.Month.Treasury.Yield[i] + 
            ...
            macro_forecasts$Time.trend[i] + sum(.position_data_processed$Net.Interest.Margin))
      
      .cols <- c("Noninterest.Nontrading.Income.Ratio", "Compensation.Noninterest.Expense.Ratio")
      .revenue_coeffs_forecast_df[.cols][i,] <- (model_coefficients["Intercept", 
                .cols] + model_coefficients["Lagged.dependent.variable", .cols] * 
            .revenue_coeffs_forecast_df[.cols][i-1,] + model_coefficients["Stock.Market.returns", 
                .cols] * macro_forecasts$Stock.Market.returns[i]
