...
RevenueForecast <- function(position_data, macro_forecasts) {
  #macro_forecasts input only needed to get appropriate start, end, and frequency
  #for outputted time series
  ...
  # create blank revenue forecast data frame (easier to work with than time series)
  .cols<-c("Interest.Earning.Assets", "Trading.Assets", "Total.Assets")
  .revenue_forecast_df <- data.frame(ts(matrix(NA, ncol=length(.cols), dimnames=list(NULL, .cols)),
  ...
  for (i in 1:.nrow) {
    if (i == 1) { # first time series entry of our forecast comes from position data
      .revenue_forecast_df$Interest.Earning.Assets[1] <- sum(position_data[.summing_vec])
      .revenue_forecast_df$Trading.Assets[1] <- position_data$Total.Trading.Assets...000.
      .revenue_forecast_df$Total.Assets[1] <- position_data$Total.Assets...000.
    }
    else { # remaining entries come from arithmetic on initial data frame entry
      .revenue_forecast_df[i, ] <- .revenue_forecast_df[1, ] * (1.0125^(i - 1))
      
    }
  } 
  return(ts(.revenue_forecast_df, 
          start = start(macro_forecasts),
          end = end(macro_forecasts),
          frequency = frequency(macro_forecasts)))
}