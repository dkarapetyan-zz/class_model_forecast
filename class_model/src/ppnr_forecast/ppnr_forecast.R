# Author: David Karapetyan This Function outputs a PPNR
...
#' @export 
#' @example
#' result_time_series <- PpnrForecast(
...
PPNRForecast <- function(position_data, model_coefficients, macro_forecasts) {
  ...
  .revenue_forecast_df <- data.frame(RevenueForecast(position_data, 
          macro_forecasts))  #df easier to work with
  .revenue_coeffs_forecast_df <- data.frame(RevenueCoeffForecast(position_data, 
          model_coefficients, macro_forecasts))
  
  
  cols <- c("Net.Interest.Income", "Non.Int...Non.Trade.Income", 
      "Trading.Income", "Compensation.Exp", "Fixed.Asset.Exp", 
      "Other.Exp", "PPNR")
  
  # create blank ppnr capital forecast time series
  .ppnr_forecast_df <- data.frame(ts(matrix(NA, ncol = length(cols)), 
          start = start(macro_forecasts), end = end(macro_forecasts), 
          frequency = frequency(macro_forecasts)))
  
  colnames(.ppnr_forecast_df) <- cols
  
  # first column of our forecast is blank.  Set up temporary
  # object without first row, populate it, then assign it to
  # ppnr_forecast's 2:end rows
  ...
  .ppnr_forecast_df_temp$Non.Int...Non.Trade.Income <-
  ...
  .ppnr_forecast_df_temp$Fixed.Asset.Exp <-
      (.revenue_coeffs_forecast_df_temp$Fixed.Asset.Noninterest.Expense.Ratio /
        400 * .revenue_forecast_df_temp$Total.Assets)
  ...
  .cols1 <- c("Net.Interest.Income", "Non.Int...Non.Trade.Income", 
      "Trading.Income")
  .cols2 <- c("Compensation.Exp", "Fixed.Asset.Exp", "Other.Exp")
  
  .ppnr_forecast_df_temp$PPNR <-
      (rowSums(.ppnr_forecast_df_temp[, .cols1]) -
